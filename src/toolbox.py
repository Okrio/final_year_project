...
def filterMarkerColour(img): 
	# Description: Applies a colour mask to filter out other objects in image 
	# Input: Image 
	# Output: Mask 
...
def findMarker(contour): 
	...
	if len(approx) == 4: 
		...
		# Check for Flags: 
		keepSolidity = solidity > 0.9
		keepAspectRatio = aspect_ratio >= 0.09 and aspect_ratio <= 0.5
		keepDims = w > 10 and h > 2
...
def detectMarker(img):
	...
	while(foundMarker == False): 
			...
			for c in contours: 
				foundMarker, approx = findMarker(c) 
			...
			if pixel_threshold < 256: 
				pixel_threshold += 1 # Note that changing this will affect the break point
